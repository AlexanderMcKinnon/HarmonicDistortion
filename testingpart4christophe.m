...
faxis =  zfs*(0:length(z)-1)/length(z);
num_peaks = 15;
...
while i <= length(wins_hann)
    ft_wins_hann{i} = sgolayfilt((20*log10(abs(fft(wins_hann{i})))), 9, 25);
    i = i + 1;
end
...
% semilogx(faxis, sgolayfilt(20*log10(abs(fft(z))), 9, 25)) ;
% xlim([15 20000]);
...
for j = [2:final_harmonic_thd]
    ...
    energyHarms{j - 1} = powerHarms{j - 1}.*faxis';
